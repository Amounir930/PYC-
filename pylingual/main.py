...
from pylingual.decompiler import decompile, load_models
...
def main(files: list[str], out_dir: Path | None, config_file: Path | None, version: PythonVersion | None, top_k: int, trust_lnotab: bool, init_pyenv: bool, quiet: bool):
    ...
    with Live(Group(Rule(), status, progress), transient=True, console=console, refresh_per_second=12.5) as live:
        ...
        # Pre-find all files and detect version if needed
        all_pyc_files = []
        for file_input in files:
            p = Path(file_input)
            if p.is_dir():
                all_pyc_files.extend(list(p.glob("**/*.pyc")))
            elif p.exists():
                all_pyc_files.append(p)
            else:
                logger.warning(f"Path {file_input} does not exist.")
        
        if not all_pyc_files:
            logger.error("No .pyc files found to decompile.")
            return
        ...
        if detected_version is None:
            ...
            try:
                first_pyc = PYCFile(all_pyc_files[0])
                detected_version = PythonVersion(first_pyc.version)
                logger.info(f"Auto-detected batch version as {detected_version}")
        ...
        for i, pyc_path in enumerate(all_pyc_files):
            ...
            log_handler.keywords = [str(pyc_path), pyc_path.name, pyc_path.with_suffix(".py").name, "decompiled_" + pyc_path.with_suffix(".py").name]
